...
void ClapTrap::beRepaired(unsigned int amount) {
	if (this->getHitPoints() > 0) {
		if (this->getEnergyPoints() == 0)
			std::cout << this->getName() << " dont have enough energypoints!" << std::endl;
		else {
			if (amount <= 0)
				...
				this->setEnergyPoints(this->getEnergyPoints() - 1);
				std::cout << this->getName() << " has repaired " << amount << " hitPoints!" << std::endl;
			}
		}
	}
	else
	...
	if (this->getHitPoints() > 0) {
		if (this->getEnergyPoints() > 0) {
			if (amount > 0) {
				...
				std::cout << this->getName() << " take " << amount << " attackdamage!" << std::endl;
